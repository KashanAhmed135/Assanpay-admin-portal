...
export function LoginPage() {
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      otpTimer.startOtp(email, data?.expiresInSec || 300, data?.resendInSec || 60)
      const debugOtp = data?.otp
      showMsg(
        'forgot',
        debugOtp ? `OTP sent (Debug): ${debugOtp}` : 'OTP sent. Please check your email.',
        'ok'
      )
      setOtpValue('')
      setResetToken('')
      ...
      showMsg('forgot', err?.data?.message || err?.message || 'Unable to send OTP.', 'err')
    }
  }
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      const data = await verifyPasswordResetOtp(email, otpValue)
      ...
      showMsg('otp', err?.data?.message || err?.message || 'OTP verification failed.', 'err')
    }
  }
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      showMsg(
        'otp',
        debugOtp ? `New OTP sent (Debug): ${debugOtp}` : 'New OTP sent. Please check your email.',
        'ok'
      )
    } catch (err) {
      showMsg('otp', err?.data?.message || err?.message || 'Unable to resend OTP.', 'err')
    }
  }
  ...
  const handleResetPassword = async () => {
    ...
    try {
      const email = otpTimer.otpState.email
      await resetPasswordWithOtp(email, resetToken, newPw)
      ...
      showMsg('reset', err?.data?.message || err?.message || 'Unable to reset password.', 'err')
    }
  }

  const verifyEnabled = otpValue.length === 5 && !otpTimer.isOtpExpired && !!otpTimer.otpState.email
  const canResetPassword = isStrongPassword(newPw) && newPw === confirmPw && confirmPw.length > 0

  return (
    <div className="login-shell min-h-screen grid place-items-center p-4 sm:p-5 md:p-6 lg:p-[22px] bg-[var(--color-bg-primary)] bg-[radial-gradient(1000px_600px_at_15%_0%,rgba(90,167,255,.18),transparent_60%),radial-gradient(900px_520px_at_85%_10%,rgba(47,208,122,.12),transparent_55%),radial-gradient(820px_520px_at_55%_45%,rgba(255,204,102,.07),transparent_60%)]">
      ...
      <div className="w-full max-w-[980px] grid grid-cols-1 lg:grid-cols-[1.05fr_0.95fr] gap-4 sm:gap-5 md:gap-6 lg:gap-[18px] rounded-[22px]">
        {/* Brand Panel */}
        <section className="border border-[var(--color-border-soft)] rounded-[22px] bg-gradient-to-b from-white/[0.06] to-white/[0.03] shadow-card overflow-hidden p-4 sm:p-5 md:p-6 lg:p-[26px] min-h-[auto] lg:min-h-[560px] relative">
          <div className="h-full flex flex-col items-center justify-center text-center gap-4 sm:gap-5 md:gap-6">
            ...
            <div className="grid gap-2">
              <h1 className="m-0 text-3xl sm:text-4xl md:text-5xl font-semibold tracking-[0.2em] text-[var(--color-text-primary)]">
                ASSANPAY
              </h1>
              <div className="text-xs sm:text-sm text-[var(--color-text-secondary)]/75">Secure access for Admin and Merchant portals.</div>
            </div>
          </div>
        </section>

        {/* Auth Panel */}
        <section className="border border-[var(--color-border-soft)] rounded-[22px] bg-gradient-to-b from-white/[0.06] to-white/[0.03] shadow-card overflow-hidden p-4 sm:p-5 md:p-6 lg:p-[26px] min-h-[auto] lg:min-h-[560px] flex flex-col justify-center">
          {/* LOGIN VIEW */}
          {view === 'login' && (
            <div className="block">
              <div className="mb-3 sm:mb-4 md:mb-[14px]">
                <h2 className="m-0 text-base sm:text-lg md:text-[18px] font-semibold">Sign in</h2>
                <p className="mt-1.5 sm:mt-2 md:mt-[6px] text-[var(--color-text-secondary)]/85 text-xs sm:text-[13px]">
              ...
              <form className="mt-3 sm:mt-4 md:mt-[14px] grid gap-3 sm:gap-4 md:gap-[12px]" onSubmit={handleLoginSubmit}>
                <div className="grid gap-1.5 sm:gap-2 md:gap-[6px]">
                  <label htmlFor="loginEmail" className="text-xs sm:text-[13px] text-[var(--color-text-secondary)]/95">
                    Email
                  </label>
                  <input
                    className="p-3 sm:p-[12px_14px] rounded-[14px] border border-[var(--color-border-soft)] bg-[var(--color-surface-muted)] text-[var(--color-text-primary)] placeholder:text-[var(--color-text-secondary)]/55 focus:outline-none focus:ring-2 focus:ring-[var(--focus-ring)] focus:ring-offset-2 focus:ring-offset-[var(--color-bg-primary)] text-sm sm:text-base"
                    id="loginEmail"
                    type="email"
                ...
                <button
                  className="p-3 sm:p-[12px_14px] rounded-[14px] border border-[var(--color-accent-border)] bg-[rgba(90,167,255,0.20)] hover:bg-[rgba(90,167,255,0.28)] cursor-pointer transition-all duration-150 whitespace-nowrap w-full text-sm sm:text-base font-medium"
                  type="submit"
                >
                  Login
                </button>
                ...
                <div className="flex gap-2 sm:gap-3 md:gap-[10px] items-center justify-between flex-wrap">
                  <span className="text-[var(--color-text-secondary)]/80 text-[10px] sm:text-xs md:text-[12px]">Need help?</span>
                  <button
                    type="button"
          ...
          {view === 'forgot' && (
            <div className="block">
              ...
              <div className="grid gap-3 sm:gap-4 md:gap-[12px]">
                <div className="grid gap-1.5 sm:gap-2 md:gap-[6px]">
                  <label htmlFor="forgotEmail" className="text-xs sm:text-[13px] text-[var(--color-text-secondary)]/95">
                    Email
                  </label>
                  <input
                    className="p-3 sm:p-[12px_14px] rounded-[14px] border border-[var(--color-border-soft)] bg-[var(--color-surface-muted)] text-[var(--color-text-primary)] placeholder:text-[var(--color-text-secondary)]/55 focus:outline-none focus:ring-2 focus:ring-[var(--focus-ring)] focus:ring-offset-2 focus:ring-offset-[var(--color-bg-primary)] text-sm sm:text-base"
                  ...
                  {otpTimer.isOtpActive && (
                    <div className="text-[var(--color-text-secondary)]/80 text-[10px] sm:text-xs md:text-[12px]">
                      OTP already sent. You can request a new OTP in{' '}
                      <span className="text-[var(--color-text-primary)]">{otpTimer.countdownText}</span>.
                    </div>
                  )}
                ...
                <div className="flex gap-2 sm:gap-3 md:gap-[10px] items-center justify-between flex-wrap">
                  ...
                  <button
                    className={`p-3 sm:p-[12px_14px] rounded-[14px] border border-[var(--color-accent-border)] bg-[rgba(90,167,255,0.20)] hover:bg-[rgba(90,167,255,0.28)] transition-all duration-150 whitespace-nowrap text-sm sm:text-base font-medium ${!otpTimer.canRequestNewOtp ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer'
                      }`}
                    type="button"
                ...
                <div className="mt-3 sm:mt-4 md:mt-[14px] border-t border-[var(--color-border-soft)] pt-3 sm:pt-4 md:pt-[12px] text-[var(--color-text-secondary)]/75 text-[10px] sm:text-xs md:text-[12px] leading-relaxed">
          ...
          {view === 'otp' && (
            <div className="block">
              <div className="mb-3 sm:mb-4 md:mb-[14px]">
                ...
                <p className="mt-1.5 sm:mt-2 md:mt-[6px] text-[var(--color-text-secondary)]/85 text-xs sm:text-[13px]">
                  ...
                  {otpTimer.otpState.expiresAt && (
                    <span className="text-[var(--color-text-primary)]">Expires in {otpTimer.countdownText}</span>
                  )}
                </p>
              </div>
              ...
              <div className="grid gap-3 sm:gap-4 md:gap-[12px]">
                <div className="grid gap-1.5 sm:gap-2 md:gap-[6px]">
                  <label htmlFor="otpInput" className="text-xs sm:text-[13px] text-[var(--color-text-secondary)]/95">
                    OTP (5 digits)
                  </label>
                  <input
                    className="p-3 sm:p-[12px_14px] rounded-[14px] border border-[var(--color-border-soft)] bg-[var(--color-surface-muted)] text-[var(--color-text-primary)] placeholder:text-[var(--color-text-secondary)]/55 focus:outline-none focus:ring-2 focus:ring-[var(--focus-ring)] focus:ring-offset-2 focus:ring-offset-[var(--color-bg-primary)] text-sm sm:text-base"
                ...
                <button
                  className={`p-3 sm:p-[12px_14px] rounded-[14px] border border-[var(--color-accent-border)] bg-[rgba(90,167,255,0.20)] hover:bg-[rgba(90,167,255,0.28)] transition-all duration-150 whitespace-nowrap w-full text-sm sm:text-base font-medium ${!verifyEnabled ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer'
                    }`}
                  type="button"
          ...
          {view === 'reset' && (
            <div className="block">
              ...
              <div className="grid gap-3 sm:gap-4 md:gap-[12px]">
                <div className="grid gap-1.5 sm:gap-2 md:gap-[6px]">
                  <label htmlFor="newPw" className="text-xs sm:text-[13px] text-[var(--color-text-secondary)]/95">
                    New password
                  </label>
                ...
                <div className="flex gap-2 sm:gap-3 md:gap-[10px] items-center justify-between flex-wrap">
                  ...
                  <button
                    className={`p-3 sm:p-[12px_14px] rounded-[14px] border border-[var(--color-accent-border)] bg-[rgba(90,167,255,0.20)] hover:bg-[rgba(90,167,255,0.28)] transition-all duration-150 whitespace-nowrap text-sm sm:text-base font-medium ${!canResetPassword ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer'
                      }`}
                    type="button"
